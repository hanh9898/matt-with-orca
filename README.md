...
- Orca mutations were unreliable under load, in parallel and occasionally alone (`worktree create` answering `runtime_unavailable` for a worktree it made, a missing `startupTerminal`, a hang), so the spawn script runs `worktree create` and `worker-start` one at a time behind a lock, bounds every `orca` call with a timeout, and tags each `worker-start` with `--retry-request` for `request-show` recovery. A `worktree create` that fails after creating its worktree is reported, never retried blind.
- A one-shot `worker-start --worktree new-top-level --agent` types the spec into an agent that is still booting, and the Enter is lost (`outcome_unknown`, spec stuck in the input box; 5 of 6 cold starts in testing). Step 4 therefore spawns in two moves and waits for the agent's input box first; the warm-signal it waits for (`bypass permissions` on screen) is specific to Claude Code as Orca launches it. Warm spawns still report `outcome_unknown` now and then (2 of 12) with the spec submitted and the agent working; the skill accepts those only on screen evidence plus a first heartbeat, a documented departure from Orca's inspect-then-stop rule.
